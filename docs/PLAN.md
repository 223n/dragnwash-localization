...
## Language picker in the Options screen (2026-09-13)

- Options rows are generated by UnityScriptableSettings' `ScriptableSettingSpawner` from `SettingsManager`'s list. The mod inserts one `SettingDropdown` subclass after the last Gameplay setting (not through `AddSetting`, whose unstable sort could reorder the game's rows), so the game builds the row with its own prefab and pad navigation. If the library is not shaped as expected, nothing is added.
...
- Follow-up (2026-09-25): working setup verified by hand on an Apple A18 Pro, macOS 27.2 ([223n/dragnwash-modframework#3](https://github.com/223n/dragnwash-modframework/issues/3), a draft issue in a fork, filed upstream as [TomXV/dragnwash-modframework#85](https://github.com/TomXV/dragnwash-modframework/issues/85) on 2026-09-26). BepInEx 5.4.23.5 `macos_universal` with `libdoorstop.dylib` and `run.sh` taken from UnityDoorstop's `ci` pre-release, which has the NeighTools/UnityDoorstop#108 fix (NeighTools/UnityDoorstop#117) but no stable release yet. The game runs as x86_64 under Rosetta, because natively on arm64 BepInEx 5.4.23.5 cannot apply Harmony patches (BepInEx/BepInEx#1402, merged but unreleased). `run_bepinex.sh` also passes on `STEAM_DYLD_INSERT_LIBRARIES`, so the Steam overlay opens. Result: `Chainloader startup complete`, and ModFramework 1.5.0 loaded all 10 plugins. The game has to be started from Steam: started from Terminal it has no Steam API, so the saves do not show.
- `install-macos.sh` now installs that setup (2026-09-26). It fetches the ci Doorstop zip pinned by SHA-256, and stops when the ci build has changed under the same URL or is gone (HTTP 404); installs then stay blocked until the pin is updated, and the message points to the issue above. `--bepinex-zip`, `--doorstop-zip` and `--framework-zip` take local copies, checked the same way. It checks that Rosetta is installed. In `run_bepinex.sh` it sets `executable_name`, `archpreference` and `target_assembly`, only on the lines that hold the defaults, adds the overlay hand-over when `run.sh` does not have it yet, and checks the result; a second run changes nothing. It also installs the ModFramework release that `mod-install.json` names. It backs up only `run_bepinex.sh` and `libdoorstop.dylib`, kept once as `<name>.dragnwash-backup` the first time it replaces them, and each `localconfig.vdf`, copied to `localconfig.vdf.dragnwash-backup` before every edit. On uninstall it also removes the Doorstop files, the backups and `preloader_*.log` along with a BepInEx it installed. It removes ModFramework only when it installed it, and then only the parts it added, which a marker file lists (a plugin counts as added only when its folder was not there before); a framework installed by hand stays, and so do BepInEx and the launch option. A switched-off part (`<name>.dll.disabled`) counts as installed: no DLL is copied next to it, and the summary says how to switch it back on (a library in the Mods screen; the core or the Preloader, which the Mods screen cannot switch, by renaming it back). The mod itself is the other way round, as in the Deck script: installing it means wanting it on, so its `.dll.disabled` goes and its lines in the Preloader's lists (`disabled.txt`, `state.txt`, `uninstall.txt`) are dropped, with the rest of each list unchanged; uninstall drops them too. When uninstall removes a Preloader it installed, those lists stay for the ModFramework parts that are left, and the summary marks a kept part that is switched off. Check reports `Chainloader startup complete`, the plugin line and a `LogOutput.log` older than the last start. `KNOWN_ISSUE` and the NeighTools/UnityDoorstop#107 patch are gone. The script was first tested only against a sandboxed game folder and Steam tree. On 2026-09-26 it was run once on the real game on the same Mac, over the setup made by hand there with the ci 4.6.0 Doorstop (whose libdoorstop.dylib is the pinned one, byte for byte), so it downloaded nothing: it kept BepInEx, the Doorstop and ModFramework 1.5.0, rewrote `run_bepinex.sh` for the full `executable_name` (keeping the backup), left its overlay hand-over as it was, installed the mod from the v1.5.0 release zip and set the launch option. Started from Steam, the translation, the Steam overlay and the F1 window worked. A fresh install on the real game is not tested, so the script stays experimental and out of the release zip. What first looked like a one-time freeze of about 15 s after installing seems to happen while the game's window is not in front: kept active, the game starts smoothly (same Mac, 2026-09-26). Where the Doorstop is downloaded from was revised the same day (see "Decision revised" below).
- Decision (2026-09-26): do not re-pin to another `ci` build; wait for stable releases. When a new `ci` build under the same URL replaces the pinned one, installs that have to download the Doorstop stay stopped until the next pin, which will be the stable UnityDoorstop 4.6.0 release once it has been tested; progress is followed in the issue above. NeighTools/UnityDoorstop#117 and NeighTools/UnityDoorstop#114 are both in the pinned `ci` 4.5.0 build (master `d8973b22`), so the stable 4.6.0 is expected to carry both. After it, the script only waits for a BepInEx release with BepInEx's arm64 fixes (BepInEx/BepInEx#1288 and BepInEx/BepInEx#1402), and switches to it; with it, the game is expected to run natively without Rosetta - not yet tested. The overlay hand-over was merged upstream on 2026-09-25 ([NeighTools/UnityDoorstop#121](https://github.com/NeighTools/UnityDoorstop/pull/121), in 4.6.0); with Doorstop 4.6.0 or later the script's overlay edit is not needed and is skipped. This decision was revised the same day (see "Decision revised" below).
- Follow-up (2026-09-26): after NeighTools/UnityDoorstop#121 was merged, UnityDoorstop master `97293a28` bumped the version to 4.6.0, so the `ci` pre-release now serves `doorstop_macos_release_4.6.0.zip`, and the pinned 4.5.0 `ci` zip is no longer downloadable (HTTP 404). Checked by hand on the same Mac: the `ci` 4.6.0 `run.sh` with only the three settings (no overlay edit) opens the Steam overlay, BepInEx and ModFramework load, and the game runs under Rosetta (x86_64). Its `libdoorstop.dylib` is byte for byte the 4.5.0 `ci` one. Decision: wait for the stable 4.6.0 release instead of pinning the `ci` 4.6.0; when it ships, pin its release URL (revised the same day; see the next entry). Until then, an install that has to download the Doorstop stops before changing anything; a game folder that already has the pinned `libdoorstop.dylib`, and `--doorstop-zip` with a saved copy of the pinned zip, still work. `install-macos.sh` now leaves a `run_bepinex.sh` that already hands on `STEAM_DYLD_INSERT_LIBRARIES` as it is (the 4.6.0 form, its own earlier edit, or the same lines put in by hand) and logs that; it edits only a `run.sh` with the old block, and still refuses one with neither. A 404 or 410 on the pinned Doorstop gets the same message as a SHA-256 mismatch: installs are blocked until a new build is tested and pinned, where to follow that, and the `--doorstop-zip` route. Other download failures keep the plain download message. The HTTP status comes from curl's `%{http_code}`, because the curl that ships with macOS (8.7.1 on the test Mac) ends a 404 over HTTP/2 with exit code 56, not 22. The Doorstop URL, the zip's SHA-256, the dylib's SHA-256 and a build label sit together, with a comment on what to set them to for the stable 4.6.0; the part of the message about the `ci` build's churn is only shown while the URL is a `ci` one, and a zip whose `libdoorstop.dylib` does not match the pinned hash is refused.
- Decision revised (2026-09-26): so that fresh installs work now, `install-macos.sh` pins the `ci` 4.6.0 build that was tried with the game, downloaded from an unchanged copy kept as a pre-release of this repository: [`doorstop-4.6.0-ci-97293a28`](https://github.com/TomXV/dragnwash-localization/releases/tag/doorstop-4.6.0-ci-97293a28). It is a copy rather than the `ci` URL because the `ci` pre-release is rebuilt under the same URL on every push to UnityDoorstop's master (10 pushes from 2026-09-20 to 2026-09-25), so a pin on that URL breaks with the next push, as the 4.5.0 one did. The copy is, byte for byte, the `doorstop_macos_release_4.6.0.zip` that upstream's Build run 36186569352 (master `97293a28`) published to `ci` on 2026-09-25 20:37 UTC: zip SHA-256 `fa3c9e4638f82873620b7e7e12ce1730ce01cdc23a7ba3bac2e6bfa31c0e929d`, `universal/libdoorstop.dylib` `5f31b9fca678536ed1636206f47b77431ac5b972ff92a0a2badf84bc065f9562` (the same dylib as the `ci` 4.5.0 one). Its tag is at the same commit, so GitHub's source archives of that tag are the build's source; the release notes say that it is unofficial, where it came from, its license (LGPL-2.1, with `LICENSE` in the zip) and the SHA-256s. The stable UnityDoorstop 4.6.0 replaces the copy once it is released and tried: upstream already has a tag `v4.6.0` at `97293a28` whose build succeeded, but no `v4.6.0` release as of 2026-09-26. For the pull request to TomXV/dragnwash-localization, the maintainer there may prefer to keep the copy in a release of their own; then, of the four pinned lines, only `DOORSTOP_URL` changes, and the links to the copy in the script's comments, the READMEs and the ROADMAP move with it. Tested with this build on the same Mac: set up by hand with only the three settings in its `run.sh` and no overlay edit, the Steam overlay opens, BepInEx and ModFramework load, and the F1 window draws text; then the script was run once over that setup, downloading nothing, and the translation, the overlay and the F1 window worked. A fresh install on the real game is still untested; in a sandbox (fake HOME and Steam tree), a fresh install with the real downloads (BepInEx, the copy, ModFramework) passed every SHA-256 check. A 404 or other bytes from the copy still stop an install before it changes anything, with the message that points to the issue above; the part about the `ci` build's churn is not shown, since the URL is not the `ci` pre-release's, and the message no longer speaks of a new build. The download list now names the repository the Doorstop comes from, taken from `DOORSTOP_URL`.
