...
- `install-macos.sh` がこの組み合わせを入れるようにした（2026-09-26）。
  ...
  Doorstopの取得元は、同じ日に見直した（下の「方針の見直し」）。
...
- 方針の見直し（2026-09-26）: 新規インストールを今すぐ動かせるように、`install-macos.sh` を、ゲームで試した `ci` 版4.6.0のビルドに固定する。
  ...
  タグは同じコミットを指すので、そのタグのGitHubのソースアーカイブが、このビルドのソースになる。
  リリースノートには、公式のものではないこと、出どころ、ライセンス（LGPL-2.1で、zipに `LICENSE` が入っている）、SHA-256を書いた。
  UnityDoorstop 4.6.0の安定版が出て試せたら、コピーをやめて、そちらに固定する。
  本家にはすでに `97293a28` を指すタグ `v4.6.0` があり、そのビルドも成功しているが、2026-09-26の時点で `v4.6.0` のリリースは出ていない。
  TomXV/dragnwash-localizationへのプルリクエストでは、向こうの管理者がコピーを自分のリリースに置きたいかもしれない。
  その場合、固定値の4行で変えるのは `DOORSTOP_URL` だけ。スクリプトのコメント、README、ROADMAPにあるコピーへのリンクも移す。
  このビルドで確かめたこと（同じMac）: `run.sh` に3つの設定だけを入れて手作業で入れた環境（オーバーレイの書き換えはなし）で、Steamオーバーレイが開き、BepInExとModFrameworkが読み込まれ、F1の窓に文字が表示された。
  そのうえでスクリプトを1回実行し（何もダウンロードしなかった）、翻訳、オーバーレイ、F1の窓が動いた。
  実際のゲームへの新規インストールは、まだ試していない。
  隔離した環境（偽のHOMEとSteamのフォルダー）では、実際のダウンロード（BepInEx、コピー、ModFramework）を使った新規インストールが、すべてのSHA-256の確認を通った。
  コピーが404になったり、中身が違ったりしたときは、今までどおりゲームのフォルダーを変えずに止まり、上のissueを案内する。
  URLが `ci` プレリリースのものではないので、`ci` ビルドが作り直されることの説明は出さない。
  止まったときの案内も、「新しいビルド」とは言わないようにした。
  ダウンロードの一覧には、Doorstopの取得元のリポジトリを、`DOORSTOP_URL` から出すようにした。
