...
指摘ごとの修正は、当初 38 本の PR に分かれていました。
...
- **`ScriptOrder.Write` のヘッダが拡張されました。**
...
| 採用 | 中 | `CONTRIBUTING.md:110` ほか | 存在しない Tools タブへ翻訳者を案内している（追跡ファイル 10 件・22 箇所。矢印は `→` が 21 箇所、`->` が 1 箇所。#54 は貢献者の導線にあたる 7 ファイル 18 箇所を直し、計画文書 3 ファイル 4 箇所は残しています） | #54 |
...
修正 PR は当初 38 本に分かれていました（当時ブランチは 40 本あり、うち 2 本は
...
> `LineMatchLayer` / `LineKey` を解決できず、CS0246 を 4 件出します。
...
- **ゲーム本体では動かしていません。** H1 のフリーズ、M1 の CSV 破損、
  M8 の `#` 始まり原文の実在は、いずれもコード上の論理と切り出し実行から導いたものです。
