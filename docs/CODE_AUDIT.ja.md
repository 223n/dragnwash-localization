...
| #24 + #26 | `install-macos.sh` | 両方採用で問題ありません |
...
- **HEAD（#7）側を採る** … `$out` が `System.Text.StringBuilder` のまま残る一方で、
  以降の 11 箇所が `$out.WriteLine(...)` を呼びます。`StringBuilder` にこのメソッドは
  存在しないため、実行時に必ず失敗します。
...
80 行下の対処表は `F1 → Translation` のまま、という自己矛盾した文書になります。
...
ゲーム内ボタンのほうは `TranslationStore.cs:392-424` が公開行を保持するため単独でも安全です。
