...
確定した指摘は 15 件で、内訳は高 3 件、中 6 件、低 3 件、整理 3 件です。詳細は[検証ラウンドの結果](#検証ラウンドの結果前半-15-件に対する反証)を参照してください。
...
> 修正 PR #12 は最終的に、`File.Replace` が `IOException` を投げた場合は
...
> 当初の記述より悪い挙動でした。
> 修正 PR #13 はこれを受けて `ReadAllText` を 1 つの文字列として渡す形に改めています。
...
> 挙げた `data/level_flow.csv` の生成元です。修正 PR #16 は 8 箇所すべてを変更しています。
...
| 採用 | 中 | `CONTRIBUTING.md:110` ほか | 存在しない Tools タブへ翻訳者を案内している（追跡ファイル 10 件・22 箇所。矢印は `→` が 21 箇所、`->` が 1 箇所。#35 は貢献者の導線にあたる 7 ファイル 18 箇所を直し、計画文書 3 ファイル 4 箇所は残しています） | #35 |
...
落ちた件数は同じですが、誤り率は 40%（6/15）から 21%（6/28）へ下がりました。落ち方の内訳が示唆的でした。
...
40 本のブランチについて、`git merge-tree --write-tree` で 60 組の候補ペア
（変更ファイルが 1 つ以上重なる組）を総当たりしました。**衝突するのは 4 組だけ**です。
ただし 4 組とも、素直な解決がそのまま誤りになります。

| 組 | ファイル | 素直に解決すると |
| --- | --- | --- |
| #7 + #11 | `tools/hash-strings.ps1` | **どちらの側を選んでも壊れます** |
| #16 + #28 | `LayoutChecker.cs` | #16 の修正が静かに戻ります |
| #35 + #36 | `CONTRIBUTING.md` / `.ja.md` | 存在しないタブ名が復活します |
| #24 + #26 | `installer/experimental/install-macos.sh` | 両方採用で問題ありません（#26 側だけを採ると `need_value` 未定義で exit 127） |

### #7 + #11 — 片側を選ぶ解決が存在しない

同じハンクに、#7 の「公開済み行の取り込みブロック」と
#11 の「`StringBuilder` → `StringWriter`」が同居します。衝突範囲の**外側**は
すべて #11 の形に自動マージされています。

- **HEAD（#7）側を採る** … `$out` が `System.Text.StringBuilder` のまま残る一方で、
  以降の 9 行（12 箇所）が `$out.WriteLine(...)` を呼びます。`StringBuilder` にこのメソッドは
  存在しないため、実行時に必ず失敗します。
- **theirs（#11）側を採る** … `$out` は正しく `StringWriter` になりますが、
  #7 の `$fromPublished` ブロックが丸ごと消えます。それでいて要約行だけは
  自動マージで残り、未定義の `$fromPublished` を参照しつつ
  「kept from the published file」と表示します。**重大度「高」の H3 が黙って再発します。**

正しい解決は、**#7 のブロックを残したうえで `$out` の 2 行だけを #11 の
`StringWriter` / `NewLine` 版に差し替える**ことです。

### #16 + #28 — 衝突マーカーが本来の行を指していない

実際にマージして確認した結果です。

```
197:<<<<<<< HEAD
198:            using (var writer = new StreamWriter(filePath, append: false, new UTF8Encoding(false)))
199:=======
     （#28 のコメントと try）
204:>>>>>>> origin/feature/fix-n13-layout-report-write-guard
206:                using (var writer = new StreamWriter(filePath, append: false, Encoding.UTF8))
```

`StreamWriter` を実際に構築している行は **206 行目、マーカーの外**にあります。
「#28 の `try` を残して HEAD 側の 1 行を捨てる」という自然な解決はコンパイルも通り、
見た目も正しいのに、**その書き込みだけ BOM 付きに戻ります**。
結果として、同じ `layout_risks.csv` が
「行があるときは BOM 付き、空のときは BOM なし」という状態になり、
L1 が解消しようとした不整合が形を変えて残ります。

解決時は 206 行目も `new UTF8Encoding(false)` に直してください。

### #35 + #36 — 新しい本文を持つ側が古いタブ名を持っている

衝突しているのは #35 がタブ名を書き換えたまさにその行で、
#36 の新しい `-Path` の段落も同じ側にあります。
「新しい内容のある側を採る」と、その 2 行だけ `F1 → Tools` に戻り、
数十行下（`CONTRIBUTING.md` は約 80 行下、`CONTRIBUTING.ja.md` は約 105 行下）の対処表は
`F1 → Translation` のまま、という自己矛盾した文書になります。

### #37 は #7 より後に

衝突はしませんが、意味的な依存があります。#37 が README に足す
「PR の前に **Hash for commit** ボタン（または `tools/hash-strings.ps1`）で作り直す」
という案内のうち、**スクリプト側は #7 が入るまで安全ではありません**。
`main` のスクリプトは出力ファイルから先頭のコメント塊しか読まないため、
F6 のダンプから手で貼った行が、作業コピーが残っていると消えます。
ゲーム内ボタンのほうは `TranslationStore.cs:391-428` が公開行を保持するため単独でも安全です。

### #29 + #31

自動マージは成立し、結果も正しい形になります。理屈のうえでは
「初回描画で `hotControl != 0` なら `_savesLevel` が `-1` のまま」という経路が残りますが、
`GUI.Button` はマウスを離した時点で `true` を返すため実際には踏みにくく、
踏んでも次の描画パスで自己修復します。
...
- **この環境ではビルドしていません。** `dotnet` が無いため、C# の指摘は読解に基づいて書きました。
  前半 15 件ぶんの修正 PR（#5〜#21）はメンテナーが Windows 11 / .NET SDK 10.0.401 で
  ビルドを確認済みです（[実機での検証](#実機での検証メンテナーによる)）。
  追加監査ぶんの C# 修正（#27〜#31）の確認状況は、各 PR のコメントを参照してください。
- **実機で動かしていません。** ゲーム本体が無いため、H1 のフリーズや M1 の CSV 破損は
  コード上の論理から導いたものであり、実測ではありません。
- **この環境では PowerShell を実行していません。** `pwsh` が無いため、H3・L3・M5 の
  PowerShell 側の挙動は、当初は読解に基づいて書きました。
  H3・M5 はその後メンテナーが PowerShell 7.6.6 で実行し、いずれも当初の記述を訂正しています。
  L3（空行の扱い）は取り下げましたが、根拠は指摘であって自分では一度も実測していません。
- **macOS 実機で通していません。** インストーラーの指摘は、該当箇所を切り出して
  `bash` と `node` でこの環境で実行し、修正前後の挙動を比較しています。
  ただし macOS 上での通しのインストール／アンインストールは行っていません。
  `shellcheck` も無いため、静的解析は経ていません。
