...
## リリース済み：v1.6.0（2026-09-26）

- ネイティブの翻訳者（DarkBloodKing さん）によるトルコ語を入れました。
- Drag'n Wash ModFramework 1.6.0 のランチャー、**更新する**、ランチャーの設定を、全部の言語パックで訳しました。
- インストーラーは、ランチャー付きの ModFramework 1.6.0 を取ってきます。
- macOS：試験版のインストールスクリプト（`installer/experimental/install-macos.sh`、223n さん作）で、まだリリースされていない Doorstop を使って、Rosetta で動かせます。
