...
namespace DragNWashLocalization
{
    ...
    internal static class InputBlocker
    {
        ...
        private static void Block()
        {
            ...
            {
                _reported = true;
                Plugin.Log($"[input] Blocking while the pointer is over the menu. Action maps suspended={SuspendedMaps.Count}");
                if (SuspendedMaps.Count == 0)
                {
                    Plugin.Log("[input] No enabled action maps were found, so gameplay input is NOT blocked.");
                }
            }
        }
        ...
        private static void SuspendActions()
        {
            ...
            {
                Plugin.Log($"[input] Could not suspend action maps: {ex.Message}");
            }
        }
        ...
        private static void RestoreActions()
        {
            ...
            {
                Plugin.Log($"[input] Could not restore action maps: {ex.Message}");
            }
            finally
            {
                SuspendedMaps.Clear();
