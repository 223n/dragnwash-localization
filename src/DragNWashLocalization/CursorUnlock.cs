...
namespace DragNWashLocalization
{
    ...
    internal static class CursorUnlock
    {
        ...
        // Set by Plugin while the menu is open; read by the Harmony postfixes.
        ...
        public static void Install(Harmony harmony)
        {
            ...
            if (t == null)
            {
                Plugin.Log("[cursor] GameStateManager not found; relying on Cursor.lockState only.");
                return;
            }
            int patched = 0;
            ...
            Plugin.Log($"[cursor] Patched {patched} GameStateManager method(s) to keep the cursor free while the menu is open.");
        }
        ...
        public static void Hold(MonoBehaviour owner)
        {
            ...
            try
            {
                ...
                if (mgr != null && req != null)
                {
                    ...
                    Plugin.Log("[cursor] Unlocked through the game's GameStateManager (player input paused).");
                    return;
                }
            }
            catch (Exception ex)
            {
                Plugin.Log($"[cursor] Game unlock failed, using Cursor.lockState instead: {ex.InnerException?.Message ?? ex.Message}");
            }
            _fallback = true;
        ...
        public static void Release()
        {
            ...
            {
                Plugin.Log($"[cursor] Release failed: {ex.InnerException?.Message ?? ex.Message}");
            }
            _ticket = null;
            if (_fallback)
            {
