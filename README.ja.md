...
> [!NOTE]
> **「Windows によって PC が保護されました」と出て何も起きない場合**
> `Install.exe` は署名のない小さなプログラムなので、初回だけ Windows SmartScreen が止めることがあります。
> 画面の **「詳細情報」→「実行」** を押してください。画面が出ずに何も起きないときは、`Install.exe` を右クリック →
> プロパティ → 一番下の **「許可する」（ブロックの解除）** にチェックして OK を押してから、もう一度ダブルクリックします。
> それでも駄目なら、展開したフォルダで PowerShell を開いて次を実行すると同じ画面が開きます：
> `powershell -ExecutionPolicy Bypass -STA -File .\installer\Installer.ps1`
...
v0.2.0 をリリース済み（進行順の翻訳ファイル、フラグ目録と編集、ワンクリックインストーラー、英語のまま遊ぶ選択肢）。BepInExプラグインの骨格、UI文字列・会話文の日本語/中国語差し替え、
