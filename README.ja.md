...
v0.5.0 をリリース済み（ゲームの Options 画面から言語を変更できるように）。v0.4.0 で 13 言語、言語ごとのフォント準備、About タブ、日英中に切り替えられるインストーラーを、v0.3.0 で Steam Deck 対応を入れました。BepInExプラグインの骨格、UI文字列・会話文の日本語/中国語差し替え、
