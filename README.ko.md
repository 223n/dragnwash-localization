...
>
> 지금 쓸 수 있는 우회 방법을 설치해 주는 **실험적(experimental)** macOS 설치 스크립트를 저장소에 넣어 두었어요: [`installer/experimental/install-macos.sh`](installer/experimental/install-macos.sh). 릴리스 zip에는 들어 있지 않아요. macOS용 BepInEx 5.4.23.5를 UnityDoorstop `ci` 프리릴리스의 Doorstop(게임에서 테스트한 4.5.0 빌드로 고정)과 함께 설치하고, 게임을 Rosetta(x86_64)로 실행하게 해요. 또 Steam 오버레이를 게임에 넘겨줘서 Shift+Tab으로 열 수 있게 해요. 이 변경은 2026-09-25에 UnityDoorstop에도 머지되어 4.6.0에 들어갔어요([NeighTools/UnityDoorstop#121](https://github.com/NeighTools/UnityDoorstop/pull/121)). Doorstop 4.6.0 이상에서는 스크립트가 따로 고칠 필요가 없어서 이 수정을 건너뛰어요. Steam Deck 스크립트처럼 Drag'n Wash ModFramework, 모드, 언어, Steam 실행 옵션도 설정해 주고, 게임을 실행한 뒤 모드가 로드됐는지 확인하는 **Check** 모드도 있어요. `ci` 빌드는 같은 URL로 다시 만들어지기 때문에, 고정한 빌드가 바뀌거나 사라질 수 있어요. **지금은 고정해 둔 4.5.0 `ci` 빌드를 더 이상 내려받을 수 없어요**(`ci` 프리릴리스가 4.6.0으로 바뀌었기 때문이에요). 그래서 Doorstop을 내려받아야 할 때는 스크립트가 게임 폴더를 바꾸지 않고 멈춰요. 이 내려받기가 필요한 설치는 다음에 다시 고정할 때까지 멈춘 상태로 있어요. 다음에 고정할 것은 다른 `ci` 빌드가 아니라, 테스트를 마친 UnityDoorstop 4.6.0 안정 릴리스예요. 진행 상황은 [223n/dragnwash-modframework#3](https://github.com/223n/dragnwash-modframework/issues/3)에서 볼 수 있어요. 고정한 Doorstop이 이미 들어 있는 게임 폴더는 영향을 받지 않고, 고정한 zip을 저장해 두었다면 `--doorstop-zip <그 파일>`을 붙여 실행해서 설치할 수 있어요. 4.6.0 안정 릴리스에는 UnityDoorstop의 수정([NeighTools/UnityDoorstop#117](https://github.com/NeighTools/UnityDoorstop/pull/117)과 [NeighTools/UnityDoorstop#114](https://github.com/NeighTools/UnityDoorstop/pull/114))이 둘 다 들어갈 것으로 예상해요. 둘 다 고정한 `ci` 빌드에 이미 들어 있기 때문이에요. 그다음에 스크립트가 기다리는 것은 BepInEx의 arm64 수정([BepInEx/BepInEx#1288](https://github.com/BepInEx/BepInEx/pull/1288)과 [BepInEx/BepInEx#1402](https://github.com/BepInEx/BepInEx/pull/1402))이 들어간 BepInEx 릴리스뿐이고, 나오면 그 릴리스로 바꿀 거예요. 그 릴리스에서는 Rosetta 없이 네이티브로 실행될 것으로 예상해요. 다만 아직 테스트하지는 않았어요. 이 조합은 Apple A18 Pro + macOS 27.2에서 직접 확인했어요. 스크립트는 같은 Mac의 실제 게임에서, 직접 설치해 둔 환경 위에 한 번 실행해 봤어요(이미 설치되어 있어서 아무것도 내려받지 않았어요). 아무것도 없는 상태에서 새로 설치하는 경우는 게임 폴더의 테스트용 복사본에서만 실행해 봤어요. 게임은 Steam에서 실행하세요. 터미널에서 실행하면 세이브를 찾지 못해요. 게임이 시작되는 동안 게임 창이 앞에 있지 않으면 타이틀 화면 전에 멈춘 것처럼 보일 수 있어요. 창을 클릭하면 계속 진행돼요(같은 Mac에서 확인했어요).
