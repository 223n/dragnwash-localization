...
> [!WARNING]
> **On macOS the mod only works with an experimental setup for now.** Drag'n Wash is built with Unity 6.3, and the Doorstop loader in BepInEx 5.4.23.5 for macOS can't hook Unity 6.3 games ([NeighTools/UnityDoorstop#108](https://github.com/NeighTools/UnityDoorstop/issues/108)). Doorstop does load into the game, but BepInEx never starts: no `BepInEx/LogOutput.log` and no `BepInEx/config` show up, and the game runs in English. We checked this on an Apple M3 Pro with macOS 26.6, both natively and under Rosetta. So far the fix ([NeighTools/UnityDoorstop#117](https://github.com/NeighTools/UnityDoorstop/pull/117)) is only in UnityDoorstop's `ci` pre-release. And running natively on Apple silicon, BepInEx 5.4.23.5 can't apply its patches; that fix ([BepInEx/BepInEx#1402](https://github.com/BepInEx/BepInEx/pull/1402)) isn't in a release yet either.
>
> For now, the repository keeps an **experimental** macOS install script that sets up a workaround: [`installer/experimental/install-macos.sh`](installer/experimental/install-macos.sh). It isn't in the release zip. It installs BepInEx 5.4.23.5 for macOS with the Doorstop from UnityDoorstop's `ci` pre-release, pinned to its 4.6.0 build, which was tested with the game, makes the game run as x86_64 under Rosetta, and hands Steam's overlay on to the game so Shift+Tab still opens it. UnityDoorstop merged that overlay change on 2026-09-25 ([NeighTools/UnityDoorstop#121](https://github.com/NeighTools/UnityDoorstop/pull/121), in 4.6.0); with Doorstop 4.6.0 or later, including the pinned build, the script's edit isn't needed and is skipped. Like the Steam Deck script, it also sets up Drag'n Wash ModFramework, the mod, the language and the Steam launch option, and it has a **Check** mode that tells you whether the mod loaded after you start the game. UnityDoorstop builds `ci` again under the same URL whenever its master branch changes, so a file pinned there can change or disappear: the 4.5.0 build the script pinned before can no longer be downloaded, since `ci` became 4.6.0 on 2026-09-25. So the script downloads the 4.6.0 build from an unchanged copy kept as a pre-release of this repository, [`doorstop-4.6.0-ci-97293a28`](https://github.com/TomXV/dragnwash-localization/releases/tag/doorstop-4.6.0-ci-97293a28), and checks its SHA-256. It will switch to the stable UnityDoorstop 4.6.0 release once that's out and tested; you can follow that in [TomXV/dragnwash-modframework#85](https://github.com/TomXV/dragnwash-modframework/issues/85). If the Doorstop it downloads doesn't match, or is no longer there (HTTP 404), the script stops before changing anything and points you to that issue. The stable 4.6.0 is expected to carry both UnityDoorstop fixes, [NeighTools/UnityDoorstop#117](https://github.com/NeighTools/UnityDoorstop/pull/117) and [NeighTools/UnityDoorstop#114](https://github.com/NeighTools/UnityDoorstop/pull/114), as both are already in the pinned build. After that, the script only waits for a BepInEx release with BepInEx's arm64 fixes ([BepInEx/BepInEx#1288](https://github.com/BepInEx/BepInEx/pull/1288) and [BepInEx/BepInEx#1402](https://github.com/BepInEx/BepInEx/pull/1402)), and switches to it; with it, the game is expected to run natively without Rosetta, which isn't tested yet. The setup, with the pinned 4.6.0 build, was checked by hand on an Apple A18 Pro with macOS 27.2. The script itself was run once on the real game on that Mac, over the setup made by hand there, so it downloaded nothing; a fresh install has only been run against a test copy of a game folder. Start the game from Steam: started from Terminal, it doesn't find your saves. While the game starts, it can look frozen before the title screen if its window isn't in front; click the window and it goes on (seen on that Mac).
...
Windows on ARM has been checked too (the game itself needs `-force-d3d11` there). macOS only works with an experimental setup for now, because of known issues on the BepInEx side (see the note under [Steam Deck / Linux](#steam-deck--linux-verified)). The BepInEx plugin skeleton, Japanese and Chinese replacement of UI and dialogue text, CJK font rendering, bulk dialogue and UI export, the in-game debug menu, layout overflow detection, the translator docs and the release workflow are all built and tested in the game.
