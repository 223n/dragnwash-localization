...
>
> For now, the repository keeps an **experimental** macOS install script that sets up a workaround: [`installer/experimental/install-macos.sh`](installer/experimental/install-macos.sh). It isn't in the release zip. It installs BepInEx 5.4.23.5 for macOS with the Doorstop from UnityDoorstop's `ci` pre-release, pinned to its 4.5.0 build, which was tested with the game, makes the game run as x86_64 under Rosetta, and hands Steam's overlay on to the game so Shift+Tab still opens it. UnityDoorstop merged that overlay change on 2026-09-25 ([NeighTools/UnityDoorstop#121](https://github.com/NeighTools/UnityDoorstop/pull/121), in 4.6.0); with Doorstop 4.6.0 or later, the script's edit isn't needed and is skipped. Like the Steam Deck script, it also sets up Drag'n Wash ModFramework, the mod, the language and the Steam launch option, and it has a **Check** mode that tells you whether the mod loaded after you start the game. UnityDoorstop builds `ci` again under the same URL, so the pinned build can change or disappear. **Right now the pinned build, the 4.5.0 `ci` build, can no longer be downloaded** (the `ci` pre-release became 4.6.0), so when the script has to download the Doorstop, it stops before changing anything. Installs that need that download stay stopped until the next pin, which will be the stable UnityDoorstop 4.6.0 release once it has been tested, not another `ci` build; you can follow that in [223n/dragnwash-modframework#3](https://github.com/223n/dragnwash-modframework/issues/3). A game folder that already has the pinned Doorstop isn't affected, and if you kept a copy of the pinned zip, `--doorstop-zip` still works. The stable 4.6.0 is expected to carry both UnityDoorstop fixes, [NeighTools/UnityDoorstop#117](https://github.com/NeighTools/UnityDoorstop/pull/117) and [NeighTools/UnityDoorstop#114](https://github.com/NeighTools/UnityDoorstop/pull/114), as both are already in the pinned `ci` build. After that, the script only waits for a BepInEx release with BepInEx's arm64 fixes ([BepInEx/BepInEx#1288](https://github.com/BepInEx/BepInEx/pull/1288) and [BepInEx/BepInEx#1402](https://github.com/BepInEx/BepInEx/pull/1402)), and switches to it; with it, the game is expected to run natively without Rosetta, which isn't tested yet. The setup was checked by hand on an Apple A18 Pro with macOS 27.2. The script itself was run once on the real game on that Mac, over the setup made by hand there, so it downloaded nothing; a fresh install has only been run against a test copy of a game folder. Start the game from Steam: started from Terminal, it doesn't find your saves. While the game starts, it can look frozen before the title screen if its window isn't in front; click the window and it goes on (seen on that Mac).
